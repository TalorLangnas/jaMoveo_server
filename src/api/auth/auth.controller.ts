...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    const { token, role, userId } = await loginUser(username, password); // Destructure the return value
    res.status(200).json({ token, role, userId }); // Send both token and role in the response
  } catch (err) {
    if (err instanceof Error) {
